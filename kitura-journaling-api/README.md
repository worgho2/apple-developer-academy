# kitura-journaling-api

Swift backend built at the Apple Developer Academy with the Kitura framework, generated from IBM's Swift Server Generator scaffold and extended with a small journaling API.

Users are stored in PostgreSQL through Swift-Kuery ORM and authenticated with HTTP Basic (`AuthRoutes.swift`). Authenticated users can create, read, update and delete "reflexions" (`ReflexionRoutes.swift`, routes under `/reflexion`), and `/user` creates and lists users. `/health` reports service status. A `/difference` route that computed days between dates is left commented out. The scaffold also ships metrics, OpenAPI docs, Docker files, Helm charts and Terraform for IBM Cloud.

## Run

Requires Swift 5.0.1 and a PostgreSQL instance reachable through the connection settings in `Sources/Application/Model/Persistence.swift`.
...
Kitura serves on port 8080 unless the environment overrides it. The original scaffold notes follow.
